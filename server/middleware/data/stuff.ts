...
stuffRouter.post(
  "/",
  async (req: Request<{}, {}, iReqStuff>, res: Response) => {
    ...
    try {
      ...
      const { rows } = await db.query(
        "INSERT INTO stuff (stuff) VALUES ($1) RETURNING *;",
        [req.body.stuff]
      );
...
stuffRouter.put("/", async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .send(
        `Error trying to get club members where club id is ${req.params.id}`
      );
  }
});

stuffRouter.delete("/", async (req: Request, res: Response) => {
  try {
    const { rows } = await db.query(
      "UPDATE stuff SET date_deleted = NOW() WHERE id = $1 RETURNING *;",
      [req.query.id]
    );
    res.status(200).send(rows);
  } catch (err) {
    ...
    res
      .status(500)
      .send(
        `Error trying to get club members where club id is ${req.query.id}`
      );
  }
});
